...
public:
    ...
    int delall();//删除全部数据
    int delpart();//删除部分数据
    int display();//数据操作菜单显示
    int find();//查找函数
    int findswitch(int t);//find的子函数
    int index();//初始菜单界面
    int input();//输入学生数据
    int insert();//插入学生数据
    int judge(); //确定或取消选择
    int part(int t);//选择操作的部分数据
    int password();//验证管理员密码
    int print(int t);//格式化输出
    int recycle();//回收站
    int reset();//重置整个系统
    int save();//保存数据
    int sort();//成绩排序
    int sortswitch(int x);//根据需求进行排序（sort的子函数）
    int run();//主函数 实现封装
};
...
int st::input()
{
    ...
    // system("cls");
    printf("◆请输入【学号】【姓名】【性别】【班级】【C语言成绩】【OS成绩】【英语成绩】【高数成绩】\n");
    ...
    for (int i = 0; i < N - tot; i++)
    {
        ...
        scanf("%s%s%s%f%f%f%f", &s[i].name, &s[i].sex, &s[i].classes, &s[i].cpp, &s[i].os, &s[i].eng, &s[i].math); //再次输入剩下数据
        s[i].ave = (s[i].cpp + s[i].os + s[i].eng + s[i].math) / 4;                //计算平均分
...
int st::print(int t)
{
    ...
    else
    {
        ...
        printf("序号\t学号\t姓名\t性别\t班级\tC语言\tOS\t英语\t高数\t平均分\n");
        for (int i = 0; i < tot; i++)
        {
            if (s[i].flag == t)
                printf("%d\t%s\t%s\t%s\t%s\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n",
                    s[i].no, s[i].num, s[i].name, s[i].sex, s[i].classes, s[i].cpp, s[i].os, s[i].eng, s[i].math, s[i].ave);
        }
        fclose(fp);
        return 1;
    }
...
int st::sortswitch(int x)
{
    ...
    for (i = 0; i < flag0 - 1; i++)
    {
        ...
        for (j = i + 1; j < flag0; j++)
        {
            ...
            if (x == 3 && s0[k].cpp < s0[j].cpp)
                k = j;
            if (x == 4 && s0[k].os < s0[j].os)
                k = j;
            if (x == 5 && s0[k].eng < s0[j].eng)
                k = j;
            if (x == 6 && s0[k].math < s0[j].math)
                k = j;
            if (x == 7 && s0[k].ave < s0[j].ave)
                k = j;
            if (k != i)
            {
                t = s0[i];
    ...
    for (i = 0; i < flag0; i++)
        printf("%d\t%s\t%s\t%s\t%s\t%.2f\t%.2f\t%.2f\t%.2f\t%.2f\n",
            s0[i].no, s0[i].num, s0[i].name, s0[i].sex, s0[i].classes, s0[i].cpp, s0[i].os, s0[i].eng, s0[i].math, s0[i].ave);
    system("PAUSE");
    return 0;
}
...
int st::insert()
{
    ...
    if (no > 0 && no <= tot)
    {
        ...
        if (strlen(s[i].num) == 1 && s[i].num[0] == '0')
            return 0;
        scanf("%s%s%s%f%f%f%f", &s[i].name, &s[i].sex, &s[i].classes, &s[i].cpp, &s[i].os, &s[i].eng, &s[i].math);
        s[i].ave = (s[i].cpp + s[i].eng + s[i].math) / 4;
        printf("\t\t\t\t******************************\n");
        printf("\t\t\t\t*    ◆确定【修改数据】吗？  *\n");
        printf("\t\t\t\t******************************\n");
        if (judge() == 1)
...
int st::change()
{
    ...
    if (no > 0 && no <= tot)
    {
        ...
        if (strlen(s[i].num) == 1 && s[i].num[0] == '0')
            return 0;
        scanf("%s%s%s%f%f%f%f", &s[i].name, &s[i].sex, &s[i].classes, &s[i].cpp, &s[i].os, &s[i].eng, &s[i].math);
        s[i].ave = (s[i].cpp + s[i].os + s[i].eng + s[i].math) / 4;
        printf("\t\t\t\t******************************\n");
        printf("\t\t\t\t*    ◆确定【修改数据】吗？  *\n");
        printf("\t\t\t\t******************************\n");
        if (judge() == 1)
...
int st::save()
{
    ...
    while (1)
    {
        ...
        if (ch == '1' || ch == '2')
        {
            ...
            if (ch == '2')
                fp = fopen("data.xls", "w");
            fprintf(fp, "序号\t学号\t姓名\t性别\t班级\tC语言\tOS\t英语\t高数\t平均分\n");
            for (int i = 0; i < tot; i++)
                if (s[i].flag == 0)
